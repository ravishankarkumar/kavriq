...
```rust
// Using a vector store or custom memory crate
let mut memory_store = MemoryStore::new();

memory_store.add_entry(MemoryEntry {
    content: "User prefers concise answers and dark mode.".to_string(),
...
// Hybrid search combining semantic similarity + filters
let relevant = memory_store.search("UI preferences", "ravi_001");
```
...
→ Continue to **[5.2 — Episodic Memory](/agentic-ai/memory-systems-rag/episodic-memory)**
