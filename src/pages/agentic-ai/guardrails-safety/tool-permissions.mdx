...
async fn execute_tool(
    ...
    let permission = permission_system.check_permission(
        tool_name,
        &args,
        &context
    ).await?;
    ...
    if !permission.granted {
        return Err(PermissionError::new(&permission.reason));
    }

    tools[tool_name].execute(args).await
}
```
...
→ Continue to **[8.3 — Human-in-the-Loop (HITL)](/agentic-ai/guardrails-safety/human-in-the-loop)**
